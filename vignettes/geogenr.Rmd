...
First, we select and download the ACS geodatabases using the functions offered by the package. When we have them in the same folder or subfolders of it, we can access the information they contain to select the one that interests us. 
...
We can query the available geodatabases by area, subject and year using the methods offered by the object.
...
In this case we are going to select the entire topic.
...
We can export reports from a theme as `rolap::flat_table` or `rolap::star_database` objects, which allow us to enrich them with additional information or work directly with OLAP tools.
