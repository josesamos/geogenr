...
Each geodatabase is structured in layers: a geographic layer, a metadata layer, and the rest are data layers. The data layers have a matrix form, the rows correspond to instances of the geographic layer, the columns to variables defined in the metadata layer, the cells are numeric values. Here are two examples:
